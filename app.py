...
def load_model():
    if not os.path.exists(MODEL_PATH):
        ...
        if not file_id:
            st.error("Format Link Google Drive tidak valid! Pastikan Anda menyalin link dengan benar.")
            st.stop()
        ...
        with st.spinner("Sedang mengunduh model dari Google Drive (ini hanya dilakukan sekali)..."):
            ...
            try:
                ...
                st.error(f"Gagal mengunduh model dari Google Drive. Pastikan akses diatur ke 'Anyone with the link'. Error: {e}")
                st.stop()
...
def predict_image(img):
    ...
    # Normalisasi pixel (0-1) dan ubah tipe data menjadi float32
    img_array = np.array(img_array, dtype=np.float32) / 255.0
    img_array = np.expand_dims(img_array, axis=0)

    prediction = model.predict(img_array, verbose=0)

    # Deteksi otomatis tipe output arsitektur model (Sigmoid vs Softmax)
    if prediction.shape[-1] == 1:
        score = float(prediction[0][0])
        # Jika hasil kelas terbalik saat Anda uji, cukup tukar posisi teks teks di bawah ini
        if score >= 0.5:
        ...
        classes_map = {0: "Tidak Retak", 1: "Retak"} # Ubah urutan angka jika kelas terbalik
...
if uploaded_file is not None:
    ...
    # Kasus 2: File berupa gambar tunggal
    else:
