...
GOOGLE_DRIVE_SHARE_LINK = "import streamlit as st
import tensorflow as tf
import numpy as np
from tensorflow.keras.preprocessing import image
from PIL import Image
...
try:
    ...
    st.info("Tip: Pastikan library tensorflow di requirements.txt sudah di-downgrade ke versi 2.15.0.")
    st.stop()
...
def predict_image(img):
    ...
    # Normalisasi pixel (skala 0-1) yang umumnya dipakai saat training
    img_array = np.array(img_array, dtype=np.float32) / 255.0
    img_array = np.expand_dims(img_array, axis=0)

    prediction = model.predict(img_array, verbose=0)

    # Deteksi struktur output biner (Sigmoid) atau multi-kelas (Softmax)
    if prediction.shape[-1] == 1:
        score = float(prediction[0][0])
        # Jika hasil kelas terbalik saat pengujian, tukar string teks di bawah ini
        if score >= 0.5:
